 ...
 *  @license   NetReviews
 *  @version   Release: $Revision: 7.8.2
 *  @date      01/04/2019
...
class AvisVerifiesModel extends ObjectModel
{
    ...
    {
        if(Configuration::get('AV_MULTILINGUE', null, null, $id_shop) === 'checked' && empty($group_name)){
            return;
        }
        
        $id_product = self::escape($id_product,false,false);
        $review_filter_orderby = self::escape(pSQL($review_filter_orderby),false,false);
        $review_filter_note = self::escape($review_filter_note,false,false);
        $current_page = self::escape($current_page,false,false);
        $reviews_per_page = self::escape($reviews_per_page,false,false);
        ...
        if (in_array($review_filter_note, $note_range)) {
            $filter .= " and rate = '".pSQL($review_filter_note)."'";
        }
        $a_sorting = explode("_", $review_filter_orderby);
        if (($a_sorting[0] == "horodate" || $a_sorting[0] == "rate") && ($a_sorting[1] == "DESC" || $a_sorting[1] == "ASC")) {
             $filter .=' ORDER BY '.$a_sorting[0].' '.$a_sorting[1];
        } elseif ($a_sorting[0] == "helpfulrating") {
            $helfulrating =", helpful-helpless as helpfulrating";
            $filter .=' ORDER BY '.$a_sorting[0].' DESC';
        }
        if ($reviews_per_page != '0') {
            $limit .=' LIMIT '.$start.', '.$end;
        }

        $sql = 'SELECT *'.$helfulrating.' FROM '._DB_PREFIX_.'av_products_reviews WHERE ref_product = \''.pSQL($id_product).'\'';

        if (!empty($group_name)) {
            if ((!empty($id_shop) && Shop::isFeatureActive()) || !empty($id_shop)) {
            ...
            $sql .= ' and iso_lang in ("'.pSQL(implode('","', $av_group_conf)).'")';
        } else {
            $sql .= " and iso_lang = '0'";
        }
        if (!empty($id_shop) && $id_shop != '1' && Shop::isFeatureActive()) {
            $multishop_condition .= ' and (id_shop = \''.pSQL(intval($id_shop)).'\')';
        } else {
        ...
        if ($getreviews == true) {
            $sql = 'SELECT COUNT(ref_product) as nbreviews FROM '._DB_PREFIX_
                .'av_products_reviews WHERE ref_product = \''.pSQL((int)$id_product).'\''.$multishop_condition.$filter;
        } else {
    ...
    public function export($header_colums, $id_shop = null)
    {
        ...
        $chosen_states = $order_statut_list;
        if (! empty($id_shop)) {
        ...
        if (version_compare(_PS_VERSION_, '1.5', '>=')) {
            ...
            $where_id_state = (! empty($order_statut_list)) ?  ' AND oh.id_order_state IN ('.$order_statut_list.')'  : '';
            $qry_sql = ' SELECT o.module, lg.iso_code, o.id_order, o.total_paid, o.id_customer, o.date_add,c.firstname, c.lastname, c.email, o.id_lang, oh.id_order_state AS current_state 
                            FROM '._DB_PREFIX_.'orders o
                            LEFT JOIN '._DB_PREFIX_.'customer c ON o.id_customer = c.id_customer
                            LEFT JOIN '._DB_PREFIX_.'lang lg ON o.id_lang = lg.id_lang
        ...
        foreach ($item_list as $item) {
            ...
            foreach ($products_in_order as $product_element) {
                ...
                if (version_compare(_PS_VERSION_, '1.5', '<')) {
                    $o_product = new Product($product_element['product_id'], false, (int)$item['id_lang']);
                ...
                if (version_compare(_PS_VERSION_, '1.5', '<')) {
                    ...
                    if(isset($o_product->id_category_default) && !empty($o_product->id_category_default)) {
                        $product_category_create = new Category($o_product->id_category_default, $item['id_lang']);
                    ...
                    if(isset($product_element['id_category_default']) && !empty($product_element['id_category_default'])){
                        $product_category_create = new Category($product_element['id_category_default'], $item['id_lang']);
                        $product_category = $product_category_create->name;
                    } else {
                        $product_category = '';
                    }
    ...
    public static function getUrlCategory($category_id, $lang_id)
    {
        $category_exist = Db::getInstance()->getRow('SELECT * FROM '._DB_PREFIX_.'category WHERE id_category  ='.(int)$category_id);
        if ($category_exist) {
            if (version_compare(_PS_VERSION_, '1.5', '>=')) {
                $o_category = new Category((int)$category_id, $lang_id, false);
            }else{
                $o_category = new Category((int)$category_id);
            }
            $protocol_link = (Configuration::get('PS_SSL_ENABLED') || self::avUsingSecureMode()) ? 'https://' : 'http://';
            $use_ssl = (Configuration::get('PS_SSL_ENABLED') || self::avUsingSecureMode()) ? true : false;
            $protocol_content = ($use_ssl) ? 'https://' : 'http://';
            $link = new Link($protocol_link, $protocol_content);
            if (version_compare(_PS_VERSION_, '1.5', '>=')) {
                $url_category = $link->getCategoryLink($o_category, null, null, null, $lang_id);
                ...
                $url_category = $link->getCategoryLink((int)$category_id);
            }
            return $url_category;
        }
    }

    public static function getUrlImageCategory($category_id, $id_image, $lang_id)
    {
        $category_exist = Db::getInstance()->getRow('SELECT * FROM '._DB_PREFIX_.'category WHERE id_category  ='.(int)$category_id);
        if ($category_exist) {
            if (version_compare(_PS_VERSION_, '1.5', '>=')) {
                $o_category = new Category((int)$category_id, false, $lang_id);
            }else{
                $o_category = new Category((int)$category_id);
            }
            $protocol_link = (Configuration::get('PS_SSL_ENABLED') || self::avUsingSecureMode()) ? 'https://' : 'http://';
            ...
            $protocol_content = ($use_ssl) ? 'https://' : 'http://';
            $link = new Link($protocol_link, $protocol_content);
            $getidimage = Image::getCover($category_id);
            // dump($category_id);
            // dump($getidimage);
            $id_cover_image = (isset($id_image) && !empty($id_image))?$id_image:$getidimage['id_image'];
            if ((version_compare(_PS_VERSION_, '1.5', '>='))) {
                
                $img_type_chosen = (version_compare(_PS_VERSION_, '1.7', '>=')) ? 'medium' : 'thickbox';
                ...
                $image_path = $link->getCatImageLink(array_values($o_category->link_rewrite)[0], $category_id, $img_type);
                // dump($o_category->link_rewrite);
                // dump($id_cover_image);
                // dump($img_type);
            } elseif ((version_compare(_PS_VERSION_, '1.4', '>='))) {
                $image_path = $link->getCatImageLink(array_values($o_category->link_rewrite)[0], $category_id.'-'.$id_cover_image, 'large');//thickbox
            } else { //1.3
                $image_path = _PS_BASE_URL_.$link->getCatImageLink($o_category->link_rewrite, $category_id.'-'.$id_cover_image, 'large');//thickbox
            }
            return $image_path;
        }
    }
